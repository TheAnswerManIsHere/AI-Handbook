...
Read `core/docs/ai-context/plan-review-contract.md` in full before doing anything else, and apply it exactly. You are on its
...
`docs/plans/PLAN_FABLE_REVIEW_FOUNDATIONS.md` in the current checkout. Read the whole file. The repository is checked out at
...
This is round 1. No previous findings were carried over, so return `previous_findings` as an empty list.

### Lens for this round: authority, bypass, and sync/bootstrap ordering

Attack from that angle specifically. It directs EMPHASIS, not scope: still read and assess the whole plan, and a serious problem outside the lens is still a finding.

Return only the JSON document matching the schema.
