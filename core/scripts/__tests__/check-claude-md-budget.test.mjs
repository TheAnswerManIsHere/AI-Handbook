...
import {
  CONFIG, checkAll, checkOne, countBytes, countLines, findRepoRoot, readBudgets,
...
test("findRepoRoot throws rather than silently passing when there is no config", () => {
  const orphan = fresh();
  try {
    assert.throws(() => findRepoRoot(orphan), /no \.agents\/machinery\.json found/);
  } finally {
...
test("the entry-point guard compares URLs, not a hand-built file:// string", () => {
  ...
  const src = readFileSync(resolve(REPO_ROOT, "core/scripts/check-claude-md-budget.mjs"), "utf8");
