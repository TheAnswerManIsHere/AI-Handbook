...
export function gapsBrief(pr, gaps) {
  const out = [
    ...
    "",
    "One honest limit on that list, and say so in your closing paragraph if it",
    "bites: it is the adjudicator's enumeration as of the LAST time it ruled. If",
    "the builder went on to fix one of these afterwards and the loop then ended",
    "without the adjudicator ruling again, the fixed one is still listed here.",
    "Nothing tracks that per defect and nothing will -- so where a block reads as",
    "though it may already have been dealt with, say that rather than asserting it",
    "is shipping.",
    "",
