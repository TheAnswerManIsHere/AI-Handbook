...
- **The batched `/maintenance` pass (the normal case): one docs-only PR per
  ...
  and fixes are pushed, the internal review tier (re-request, and the strict
  internal rubric deciding whether anything is written; David 2026-08-21)
  rather than an unreviewed merge. **No
