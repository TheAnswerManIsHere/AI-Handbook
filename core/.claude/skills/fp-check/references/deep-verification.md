...
**That is a standing dispatch BAR** under `CLAUDE.md`'s *Whether a judgement dispatches is fixed in advance* — pre-registered here in writing, not decided at dispatch time, and **not overridden by the 2026-08-17 always-Fable rule for adjudications**. That rule governs which model a dispatch uses, never whether one happens. Removing this bar is a contract change that ships through the ordinary PR path, never a mid-dispatch call. (Codex, #504 round 2, which found this bar missing from that section's registry.)
