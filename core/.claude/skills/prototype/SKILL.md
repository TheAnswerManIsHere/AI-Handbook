...
- **A prototype gets no review loop, and no quality bar but the user's** (David, 2026-09-26): *"We must not care about security, performance, observability, documentation, etc. until we've locked a design and the feature spec."* No plan, no plan-review loop, no shared-judgement round, no assessor dispatch, no translation, no `/simplify`, no tests. The only feedback that changes a prototype is David's or the product owner's, about how it feels to use. Codex's automatic pass on the draft `[PROTOTYPE]` PR is unavoidable and is read for nothing: its threads are resolved with one line naming this rule, and no finding on a prototype starts a fix. What a prototype learns reaches the product through the normal pipeline once the design and the spec are locked, and that is where every one of those concerns is paid for.
